...
void login_C(thData tdL);
char *login_BD(thData tdL, char username[30], char password[30]);
int getUser_C(thData tdL);
int register_C(thData tdL);
char *register_BD(char username[30], char password[30]);
int addAdmin_C(thData tdL);
char* addAdmin_BD(char username[30]);
int banUserVote_C(thData tdL);
char *banUserVote_BD(char username[30]);
void addSong_C(thData tdL);
char *addSong_BD(song *s, char *genres);
void addGenre_BD(int id, char genre[10], MYSQL *conn);
song *getSongByNameArtist_BD(char name[30], char artist[30]);
song *getSongById_BD(int id);
song *mostRecentSong_BD();
char *getTop_BD(char *type);
void getTop_C(thData tdL);
void getSongByNameArtist_C(thData tdL);
char *addVote_BD(thData tdL, float vot, int song_id);
char *addComment_BD(thData tdL, char comentariu[100], int song_id);
char *deleteSong_BD(char name[30], char artist[30]);
...
MYSQL *getConnection()
{
        ...
        if (mysql_real_connect(conn, "localhost", "root", "root", "proiect", 0, NULL, 0) == NULL)
        {
...
void login_C(thData tdL)
{
        ...
        //VERIFICARE + RASPUNS
        char *mesaj = login_BD(tdL, username, parola); //verific datele
...
int register_C(thData tdL)
{
        ...
        char *mesaj = register_BD(username, parola);
...
int banUserVote_C(thData tdL)
{
        ...
        char *mesaj = banUserVote_BD(username);
        if (write(tdL.cl, mesaj, 50) <= 0)
        {
                perror("[server]Eroare la write() catre client.\n");
                return -1;
...
int addAdmin_C(thData tdL)
{
        ...
        char *mesaj = addAdmin_BD(username);
        if (write(tdL.cl, mesaj, 50) <= 0)
        {
                perror("[server]Eroare la write() catre client.\n");
                return -1;
...
char *addSong_BD(song *s, char *genres)
{
        ...
        {
                song *last = mostRecentSong_BD();
                if (strchr(genres, ',') == NULL)
                        addGenre_BD(last->id, genres, conn);
...
char *addVote_BD(thData tdL, float vot, int song_id)
{
        ...
        char *mesaj2 = login_BD(tdL, tdL.client->name, tdL.client->password);
...
void addGenre_BD(int id, char genre[30], MYSQL *conn)
{
        // MYSQL *conn = getConnection();
        if (conn != NULL)
        {
...
void getSongByNameArtist_C(thData tdL)
{
        ...
        song *s = getSongByNameArtist_BD(name, artist);
        //printf("%s\n", s->artist);
        fflush(stdout);
        ...
        if (s->id != 0)
        {
                ...
                while (1)
                {
                        ...
                        if (optiune == 1)
                        {
                                ...
                                char *rasp = addVote_BD(tdL, vot, s->id);
                                printf("\n%s", rasp);
                        ...
                        else if (optiune == 2)
                        {
                                ...
                                char *rasp = addComment_BD(tdL, comentariu, s->id);
                                printf("\n%s", rasp);
                                if (write(tdL.cl, rasp, 50) <= 0)
                                {
                                        perror("[server]Eroare la write() catre server.\n");
...
song *getSongByNameArtist_BD(char name[50], char artist[50])
{
        MYSQL *conn = getConnection();
...
void deleteSong_C(thData tdL)
{
        ...
        char *rasp = deleteSong_BD(name, artist);
        if (write(tdL.cl, rasp, 50) <= 0)
        {
                perror("[server]Eroare la write() catre client.\n");
                return;
